...
```
dbx execute --cluster-name=<name of interactive cluster> --job=dbx_local_test-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=dbx_local_test-sample-integration-test --files-only
dbx launch --job=dbx_local_test-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `dbx_local_test/__init__.py` file and tag the current code version:
```
